...
*
*   This templated array allows you to alter several behaviors, including what memory source it allocates from. As it accepts any allocator that satisfies nkr::allocator_i, you can use publically global or privately local allocators. It stores an instance of the allocator in the footprint of the array, so for global allocators this is trivially small but with local allocators the memory used would be privately all encompassing and thus completely dedicated to the array.
* 
