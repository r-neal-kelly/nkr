...
namespace nkr { namespace allocator {
    ...
    TEST_SUITE("heap_zeros_t<unit_p>")
    {
    #define types   \
        std_bool_t, \
        bool_t,     \
    ...
    #define volatiles           \
        ...
        TEST_SUITE("aliases")
        {
            TEST_SUITE("unit_t")
            {
                ...
                {
                    static_assert(std::same_as<heap_zeros_t<unit_p>::unit_t, unit_p>);
                }
            }
            ...
            TEST_SUITE("units_t")
            {
                TEST_CASE_TEMPLATE("should equal unit_p*", unit_p, types, volatiles)
                {
                    static_assert(std::same_as<heap_zeros_t<unit_p>::units_t, unit_p*>);
                }
            }
        }
        ...
        TEST_SUITE("methods")
        {
            TEST_SUITE("Allocate(units_t)")
            {
                ...
                {
                    using heap_zeros_t = heap_zeros_t<unit_p>;
                    using unit_t = heap_zeros_t::unit_t;
                    using units_t = heap_zeros_t::units_t;
                ...
                TEST_CASE_TEMPLATE("should initialize all bytes to equal 0", unit_p, types, volatiles)
                {
                    using heap_zeros_t = heap_zeros_t<unit_p>;
                    using unit_t = heap_zeros_t::unit_t;
                    using units_t = heap_zeros_t::units_t;
                    ...
                    count_t unit_count = Random<count_t>(1, 16);
                    allocator.Allocate(units, unit_count);
                    for (index_t idx = 0, end = unit_count * sizeof(unit_t); idx < end; idx += 1) {
                        WARN(reinterpret_cast<byte_t*>(const_cast<std::remove_cv_t<unit_t>*>(units))[idx] == 0);
                    }
            ...
            TEST_SUITE("Allocate(pointer_t)")
            {
                ...
                TEST_CASE_TEMPLATE("should initialize all bytes to equal 0", unit_p, types, volatiles)
                {
                    ...
                    allocator.Allocate(units, unit_count);
                    for (index_t idx = 0, end = unit_count * sizeof(unit_t); idx < end; idx += 1) {
                        WARN(reinterpret_cast<byte_t*>(const_cast<std::remove_cv_t<unit_t>*>(units.units))[idx] == 0);
                    }
            ...
            TEST_SUITE("Reallocate(units_t)")
            {
                ...
                TEST_CASE_TEMPLATE("should initialize all bytes to equal 0", unit_p, types, volatiles)
                {
                    ...
                    count_t new_unit_count = Random<count_t>(17, 32);
                    allocator.Reallocate(units, new_unit_count);
                    for (index_t idx = 0, end = new_unit_count * sizeof(unit_t); idx < end; idx += 1) {
                        WARN(reinterpret_cast<byte_t*>(const_cast<std::remove_cv_t<unit_t>*>(units))[idx] == 0);
                    }
